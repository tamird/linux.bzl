...
// discovery and replay actions only consume its verified, immutable outputs.
func runNativeConfig(opts nativeConfigRunOptions) error {
	...
	if err := os.WriteFile(filepath.Join(work, ".config"), []byte(resolvedConfigSeed(opts.resolved)), 0o644); err != nil {
		return err
	}
	...
	command := exec.Command(executable, "--syncconfig", "Kconfig")
	command.Dir = work
	...
	if output, err := command.CombinedOutput(); err != nil {
		return fmt.Errorf("selected source conf --syncconfig: %w\n%s", err, output)
	}
	projection, err := readNativeConfigProjection(work)
	if err != nil {
		return err
	}
	if err := projection.verify(opts.evaluation.tree, opts.resolved); err != nil {
		return err
	}

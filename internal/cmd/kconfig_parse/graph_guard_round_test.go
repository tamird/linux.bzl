...
func TestKbuildEmptyFeatureRoundTripPreservesMeasuredPlanEvidence(t *testing.T) {
	...
	if !sameKbuildSelectedProbePlan(first, measured) {
...
func TestKbuildGraphGuardRoundsMeasureSelectedFrontierAndFailOnExhaustion(t *testing.T) {
	...
	if err != nil || !sameKbuildGraphGuardPlan(first, stack) {
	...
	if err != nil || !sameKbuildGraphGuardPlan(second, converged) {
		t.Fatalf("converged source guard = %v, %v", converged, err)
	}
}

func TestSelectedSourceAndFeatureRoundsBoundFourNewProducers(t *testing.T) {
	for _, stage := range []string{"source-output", "feature-dump"} {
		t.Run(stage, func(t *testing.T) {
			var measured *kconfig.ProbePlan
			for index := range 4 {
				writer := familyCompilerGuardPlanForTest(t, stage+" selected producer "+strconv.Itoa(index))
				next, err := nextKbuildSelectedProbeRound(measured, writer, stage, false)
				if err != nil || len(next.Terminal) != index+1 {
					t.Fatalf("round %d lost a selected producer: %v, %v", index, next, err)
				}
				if measured != nil {
					if err := validateEarlierKbuildSelectedProbeRound(measured, next, stage); err != nil {
						t.Fatalf("round %d lost measured prior frontier: %v", index, err)
					}
				}
				measured = next
			}
			fifth := familyCompilerGuardPlanForTest(t, stage+" selected producer 4")
			if _, err := nextKbuildSelectedProbeRound(measured, fifth, stage, true); err == nil || !strings.Contains(err.Error(), "rounds exhausted") {
				t.Fatalf("unmeasured producer at the last round must fail: %v", err)
			}
			stable, err := nextKbuildSelectedProbeRound(measured, familyCompilerGuardPlanForTest(t), stage, true)
			if err != nil || !sameKbuildSelectedProbePlan(stable, measured) {
				t.Fatalf("the fifth round must prove stable membership: %v, %v", stable, err)
			}
		})
	}
}

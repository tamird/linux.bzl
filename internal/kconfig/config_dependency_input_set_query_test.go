...
func TestConfigDependencyInputSetQueryMatchesNaiveProvenanceAndUses(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "generated config producer", producer: "generated-config"},
		{name: "unknown source", source: "src-99999999"},
		{name: "unknown producer", producer: "missing"},
		{name: "invalid producer slot", producer: "generated-config", slot: 1},
...
func TestConfigDependencyInputSetQueryPreservesLexicalEvents(t *testing.T) {
	for _, firstError := range []bool{false, true} {
		t.Run(fmt.Sprintf("firstError=%v", firstError), func(t *testing.T) {
			plan := configDependencyInputSetQueryTestPlan()
			...
			marker, used, err := query.configUse(root, true)
			if firstError {
				if err == nil || used {
...
func TestConfigDependencyInputSetQueryRejectsMalformedRoots(t *testing.T) {
	for _, name := range []string{"missing root", "invalid digest", "non-root depth", "missing child"} {
		t.Run(name, func(t *testing.T) {
			...
			assertConfigDependencyInputSetQueryMatchesNaive(t, plan, root)
			if _, _, err := newConfigDependencyInputSetQuery(plan).configUse(root, true); err == nil {
	...
	t.Run("cycle", func(t *testing.T) {
		...
		if _, _, err := query.configUse(root, true); err == nil || !strings.Contains(err.Error(), "cycle") {
	...
	check := func() {
		for _, root := range roots {
			...
			for _, scanAll := range []bool{false, true} {
				if _, used, err := query.configUse(root, scanAll); err != nil || !used {
					t.Fatalf("configUse = %v, %v, want used=true", used, err)
				}
			}
		}
	}
...
func configDependencyInputSetQueryTestPlan() *ActionPlan {
	return &ActionPlan{
		...
		Nodes: []ActionPlanNode{
			...
			{ID: "generated-config", Outputs: []ActionPlanOutput{{Tree: "prep", Path: "include/generated/autoconf.h"}}},
		},
	}
}
...
func naiveConfigDependencyInputSetQuery(plan *ActionPlan, root string, stage, scanAll bool) (string, bool, error) {
	if root == "" {
		return "", false, nil
	}
...
func assertConfigDependencyInputSetQueryMatchesNaive(t *testing.T, plan *ActionPlan, root string) {
	t.Helper()
	query := newConfigDependencyInputSetQuery(plan)
	_, wantStaged, wantErr := naiveConfigDependencyInputSetQuery(plan, root, true, false)
	...
	for _, scanAll := range []bool{false, true} {
		wantMarker, wantUsed, wantErr := naiveConfigDependencyInputSetQuery(plan, root, false, scanAll)
		gotMarker, gotUsed, gotErr := query.configUse(root, scanAll)
		if gotMarker != wantMarker || gotUsed != wantUsed || fmt.Sprint(gotErr) != fmt.Sprint(wantErr) {
			t.Fatalf("scanAll=%v: use = %q, %v, %v; naive = %q, %v, %v",
				scanAll, gotMarker, gotUsed, gotErr, wantMarker, wantUsed, wantErr)
		}
	}
}

...
func TestAnalyzeCompactKbuildLinkVmlinuxPhasesSourceSpans(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if phases.InitMakeLine <= phases.VersionEndLine ||
				phases.ModpostMakeLine <= phases.ObjectEndLine ||
				phases.InitMakeLine >= phases.ObjectStartLine ||
				phases.ModpostMakeLine >= phases.FinalStartLine {
				t.Fatalf("wrong phase ordering: %+v", phases)
			}
			if phases.InitMake != `${MAKE} -f "${srctree}/scripts/Makefile.build" obj=init need-builtin=1` ||
				phases.ModpostMake != `${MAKE} -f "${srctree}/scripts/Makefile.modpost" MODPOST_VMLINUX=1` {
				t.Fatalf("recursive Make invocations differ from source: %q, %q", phases.InitMake, phases.ModpostMake)
			}
		})
	}
}

...
`)
	...
	first, ok := KbuildControlEvaluationRecipeSnapshot(evaluation, target, ruleIndex, 0)
	if !ok {
		t.Fatal("recursive scripts/basic source line has no selected immutable frontier")
	}
	second, ok := KbuildControlEvaluationRecipeSnapshot(evaluation, target, ruleIndex, 1)
	if !ok {
...
`)
	...
	if err := stepper.BeginTarget(target, makeWord, ""); err != nil {
		t.Fatal(err)
	}
	line, err := stepper.BeforeRecipe(KbuildSelectedControlRecipeLine{
...
`)
			...
			for _, selected := range []struct {
				...
				if err := stepper.BeginTarget(selected.target, selected.target, "all"); err != nil {
					t.Fatal(err)
				}
				line, err := stepper.BeforeRecipe(KbuildSelectedControlRecipeLine{
			...
			readSnapshot, ok := KbuildControlEvaluationRecipeSnapshot(
...
`)
			...
			if err := stepper.BeginTarget(header, header, ""); err != nil {
				t.Fatal(err)
			}
			...
			firstView, ok := KbuildControlEvaluationRecipeSnapshot(evaluation, header, ruleIndex, 0)
			if !ok {
				t.Fatal("first filechk line lacks its selected source view")
			}
			lastView, ok := KbuildControlEvaluationRecipeSnapshot(evaluation, header, ruleIndex, 1)
...
`)
			...
			before, ok := KbuildControlEvaluationRecipeSnapshot(evaluation, header, ruleIndex, 0)
			if !ok || len(before.Reads()) == 0 || before.Reads()[0].Exists {
				t.Fatalf("before-writer Make read = %#v, want virtual absence", before.Reads())
			}
			after, ok := KbuildControlEvaluationRecipeSnapshot(evaluation, header, ruleIndex, 2)
...
`)
	...
	snapshot, ok := KbuildControlEvaluationRecipeSnapshot(evaluation, list, selectedControlTestRuleIndex(t, profile, list), 0)
	if !ok || snapshot == nil {

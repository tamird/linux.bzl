...
`), "Kconfig", Options{})
	...
	if tree.defconfigSym != tree.Symbols["DEFCONFIG_LIST"] || !tree.Symbols["DEFCONFIG_LIST"].NoWrite {
		t.Errorf("defconfig_list = %#v, want no-write default-config symbol", tree.defconfigSym)
	}
	if tree.modulesSym != tree.Symbols["MODULES"] {
		t.Errorf("modules = %#v, want MODULES", tree.modulesSym)
	}
	if !tree.Symbols["ENABLED_BY_ALLNOCONFIG"].AllNoConfigY {
		t.Error("allnoconfig_y property was lost")
	}
